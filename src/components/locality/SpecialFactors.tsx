...
import { Trees, Maximize2, Milestone, Plus, ChevronDown, ChevronUp } from "lucide-react";
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      >
        <div className="flex flex-col items-start">
          <h2 className="text-xl font-bold text-gray-900">Special Factors</h2>
          {!isOpen && (
            <p className="text-sm text-gray-500 mt-1">
              Park, Corner & Road premiums (Up to +{combinedBonus}% bonus)
            </p>
          )}
        </div>
      ...
      {isOpen && (
        <div className="p-6 pt-0 border-t border-gray-50">
            ...
            {factors.map((factor) => {
              ...
              return (
                <div key={factor.id} className="text-center">
                  ...
                  <Badge className={`${factor.color} border text-sm font-semibold mb-2`}>
                    +{factor.percentage}%
                  </Badge>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600">
                  Park-facing + Corner Plot = <span className="font-bold text-green-600">+{combinedBonus}%</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 p-4 bg-blue-50 rounded-xl border border-blue-200">
            <h4 className="font-semibold text-gray-900 mb-2">Road Width Adjustments</h4>
            <div className="text-sm text-gray-700 space-y-1">
              <div>• Up to 6m: Base rate (no adjustment)</div>
              <div>• 6-15m width: +{locality.road_6to15_factor * 100}% premium</div>
              <div>• Above 15m width: +{locality.road_15plus_factor * 100}% premium</div>
            </div>
          </div>
        </div>
      )}
